...
main (v3.2.0) 🎯 現在地
  ...
  └─ PR #63: 算数の学ぶ機能（LessonContent） ✅ マージ済み
     ↓ [Phase 4.3 進行中]
  └─ PR #80: グローバルランキング機能（複数タブ表示） ✅ マージ済み
```
...
## 🚀 Phase 4.3: グローバルランキング機能（マルチアプリ統合）🔄 進行中

### 実装状況: 複数タブ表示対応 ✅ (2026-09-11)

**PR #80**: feat: グローバルランキング機能 - 複数タブ表示対応 ✅ マージ完了
...
**実装ファイル**:
```
lib/screens/ranking_screen.dart (改善)
  - TabBar: グローバル・週間・月間・フレンド 4タブ対応
  - 各タブ独立した fetchXxxRanking() 呼び出し
  - SwipeRefresh による個別リロード機能
```

**機能**:
- ✅ グローバルランキング: 全ユーザー上位100位
- ✅ 週間ランキング: 週間スコアベース（リセット毎週）
- ✅ 月間ランキング: 月間スコアベース（リセット毎月）
- ✅ フレンドランキング: フレンドユーザーのみ表示

#### 2. ユーザー詳細情報画面 ✅
**新規ファイル**:
```
lib/screens/user_ranking_detail_screen.dart
  - プロフィールセクション (アバター・名前・順位)
  - ランキング統計セクション (正答率・速度・問題数など)
  - スコア詳細セクション (グローバル/週間/月間スコア)
  - ユーザー情報セクション (ID・プライバシー設定・登録日)
```
...
- ✅ Build iOS (unsigned): SUCCESS
- ✅ Mergeable State: CLEAN
- ✅ PR マージ完了 (commit: 53811d6)

### 次フェーズ (Phase 4.4, 4.5, 4.6)
- [ ] フレンド検索・申請機能（Firebase 接続）
- [ ] デイリーミッション統一
- [ ] 保護者ゲート・スクリーンタイム制限
...
**社会アプリ** (`social_quiz_app`) が既に以下を実装しているため、リファレンス：
- RevenueCat SDK 統合完了
- Cloud Functions による決済代理処理
- マルチプレイ・レート計算システム

**sansu-kore** の既存実装：
- Google Mobile Ads 統合
- バッジ・キャラクター・ランキング（ローカル）
- フレンド UI（Firebase 未接続）

### チェックリスト

- [ ] shared_core の `Character` モデル拡張
- [ ] shared_core の `Badge` モデル拡張（教科別）
- [ ] shared_core に `Ranking` provider 追加
- [ ] 各アプリの RevenueCat 導入
- [ ] Firestore スキーマ設計（ランキング・フレンド）
- [ ] マルチプレイ API 実装（Cloud Functions）
- [ ] CI/CD パイプライン統一化（fastlane）
