...
final userRegisteredAtOverrideProvider =
  ...
  final registeredAtString =
      prefs.getString('sansu_user_registered_at');
  ...
  await prefs.setString('sansu_user_registered_at', now.toIso8601String());
  return now;
});

/// sansu-kore 用：サブスク購読状態プロバイダー
...
/// sansu-kore 用：クイズアクセス状態を override
...
/// クイズへのアクセス可否（簡略版）
final canAccessSansuQuizzesProvider = FutureProvider<bool>((ref) async {
  final registeredAt =
      await ref.watch(userRegisteredAtOverrideProvider.future);
  final isSubscribed =
      await ref.watch(subscriptionStatusOverrideProvider.future);
...
/// ペイウォール表示が必要か
final shouldShowSansuPaywallProvider =
    FutureProvider<bool>((ref) async {
  final canAccess =
      await ref.watch(canAccessSansuQuizzesProvider.future);
  return !canAccess;
});

/// 無料期間の残り日数
final remainingSansuFreeDaysProvider =
